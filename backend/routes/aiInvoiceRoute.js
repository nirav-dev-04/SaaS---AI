...
if (!API_KEY) {
  console.warn("Not found Gemini Api key!");
}
  ...
  try {
    if (!API_KEY) {
      return res.status(500).json({
        message: "server configuration is failed!",
        sucess: false,
      });
    }
    ...
    if (!prompt || !prompt.trim()) {
      return res.status(400).json({
        message: "prompt Text Requried",
        sucess: false,
      });
    }

    const fullPrompt = buildInvoicePrompt(fullPrompt);
    let lastErr = null;
    let lastText = null;
    let usedModel = null;
    ...
    try {
      ...
      console.log("Failed to parse  json from api respnce: ", parseErr, {
        model: usedModel,
        jsonText,
      });
